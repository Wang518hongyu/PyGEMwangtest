...
def get_shean_glacier_nos(region_no, number_glaciers=0, option_random=0):
    ...
    csv_path = current_directory + '/workspace/MassBalance/hma_mb_paper-1.0/data/mb/hma_mb_20190215_0815_std+mean_all_filled_bolch.csv'
...
if option_calibration == 2:
    ...
    precfactor_gamma_region_dict_fullfn = main_directory + '/workspace/Output/precfactor_gamma_region_dict.csv'
    precfactor_gamma_region_df = pd.read_csv(precfactor_gamma_region_dict_fullfn)
    precfactor_gamma_region_dict = dict(zip(precfactor_gamma_region_df.Region.values,
    ...
    tempchange_norm_region_dict_fullfn = main_directory + '/workspace/Output/tempchange_norm_region_dict.csv'
    tempchange_norm_region_df = pd.read_csv(tempchange_norm_region_dict_fullfn)
    tempchange_norm_region_dict = dict(zip(tempchange_norm_region_df.Region.values,
...
# width_calving_dict_fullfn = main_directory + '/workspace/Calving_data/calvingfront_widths.csv'
...
# frontalablation_k0dict_fullfn = main_directory + '/workspace/Calving_data/frontalablation_k0_dict.csv'
...
# # Dates
eraint_start_date = '19800101'
eraint_end_date = '19810101'
# Resolution
grid_res = '0.5/0.5'
...
# ERA5
era5_fp = main_directory + '/workspace/Climate_data/ERA5/'
...
eraint_lr_fn = ('ERAInterim_lapserates_' + eraint_start_date + '_' + eraint_end_date + '_opt' + str(option_lr_method) +
                '_world.nc')
# assert os.path.exists(eraint_fp)
# assert os.path.exists(eraint_temp_fn)
# assert os.path.exists(eraint_prec_fn)
# assert os.path.exists(eraint_elev_fn)
# assert os.path.exists(eraint_lr_fn)

# CMIP5 (GCM data)
cmip5_fp_var_prefix = main_directory + '/workspace/Climate_data/cmip5/divid/rcp26_r1i1p1_monNG-20201216T075646Z-001/'
cmip5_fp_var_ending = '_r1i1p1_monNG/'
cmip5_fp_fx_prefix = main_directory + '/workspace/Climate_data/cmip5/divid/rcp26_r0i0p0_fx-20201216T061931Z-001/'
...
rgi_fp = main_directory + '/workspace/IceThickness/00_rgi60/00_rgi60_attribs/'
# Column names
...
if hyps_data == 'oggm':
    ...
    hyps_filepath = main_directory + '/workspace/IceThickness/output/'
    ...
    hyps_colsdrop = ['RGIId']
    # Thickness data
    thickness_filepath = main_directory + '/workspace/IceThickness/output/'
    ...
    # Width data
    width_filepath = main_directory + '/workspace/IceThickness/output/'
...
# Debris datasets
debris_fp = main_directory + '/workspace/IceThickness/output/'
...
# User specified start/end dates（作者指定的日期）
...
# ===== SHEAN GEODETIC =====
shean_fp = main_directory + '/workspace/MassBalance/hma_mb_paper-1.0/data/mb/'
...
# ===== REGIONAL DATA =====
# Regional data refers to all measurements that have lumped multiple glaciers together（区域数据指的是将多个冰川聚集在一起的所有测量数据
# ）
