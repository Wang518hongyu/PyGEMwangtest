...
#        RGI region number for supercomputer
    ...
    Returns
    -------
    Object containing arguments and their respective values.（包含参数及其各自值的对象。）
    """
    ...
    (glac_bin_temp, glac_bin_prec, glac_bin_acc, glac_bin_refreeze, glac_bin_snowpack, glac_bin_melt,
     ...
     offglac_wide_refreeze, offglac_wide_melt, offglac_wide_snowpack, offglac_wide_runoff) = (
        massbalance.runmassbalance(modelparameters, glacier_rgi_table, glacier_area_initial, icethickness_initial,
                                   ...
                                   debug=False)) 
    # Option to return must melt condition（选择返回必须熔化的条件）
    if return_tc_mustmelt == 1:
        # Climatic mass balance of lowermost bin must be negative at some point（在某一点上，最下层仓的气候质量平衡必须为负）
        ...
        # Compute glacier volume change for every time step and use this to compute mass balance（计算冰川体积从而计算物质平衡）
...
def retrieve_priors(modelparameters, glacier_rgi_table, glacier_area_initial, icethickness_initial, width_initial, 
    ...
    """
    Calculate parameters for prior distributions for the MCMC analysis（为MCMC分析计算先验分布的参数）
    ...
    # ----- TEMPBIAS: max accumulation -----（温度偏差）
    # Lower temperature bound based on max positive mass balance adjusted to avoid edge effects（基于最大正质量平衡的温度下限调整以避免边缘效应）
    ...
    # Looping forward and backward to ensure optimization does not get stuck（向前和向后循环以确保优化不会卡住）
    ...
    # use absolute value because with area evolving the maximum value is a limit（使用绝对值，因为随着面积的发展，最大值是一个限制）
    ...
    # Looping backward for tempchange at max loss（在最大损耗下反向循环温度变化）
    while vol_remaining == 0:
    ...
    while nyears_negmbclim < 10 and modelparameters[7] < tempchange_max_loss:
        ...
        # Record if using big step and no there is no melt or if using small step and there is melt（记录是否使用大步进没有熔融或使用小步进有熔融）
...
def main(list_packed_vars):
    ...
    #           the simulations.（用MCMC方法确定温度变化、ddfsnow和prefactor三个参数的后验概率分布。然后创建一个从这些分布均匀采样的参数集集合，并输出这些参数集及其相应的质量平衡，以用于模拟）
    if input.option_calibration == 2:
        ...
        def run_MCMC(precfactor_disttype=input.precfactor_disttype,
                     ...
                     precfactor_mu=input.precfactor_mu,
                     precfactor_sigma=input.precfactor_sigma,
            ...
            （通过设置质量平衡函数的先验分布和校准三个参数的概率分布来运行MCMC算法。）
            ...
            # Define deterministic function for MCMC model based on our a priori probobaility distributions.（基于先验可测分布定义MCMC模型的确定性函数）
            @deterministic(plot=False)
            def massbal(tempchange=tempchange, precfactor=precfactor, ddfsnow=ddfsnow):
                """
                Likelihood function for mass balance [mwea] based on model parameters（基于模型参数的质量平衡似然函数）
                """
            ...
            # ===== POTENTIAL FUNCTION =====
            # Potential functions are used to impose additional constrains on the model（势函数用于对模型施加附加约束）
            @pymc.potential
            ...
            # ===== OBSERVED DATA =====（观测数据）
            #  Observed data defines the observed likelihood of mass balances (based on geodetic observations)（观测数据定义了观测到的质量平衡可能性（基于大地测量观测））
...
#            if debug:
            ...
            # ===== RUN MARKOV CHAIN MONTE CARLO METHOD ====================（运行马尔可夫链蒙特卡罗方法）
            if icethickness_initial.max() > 0:
                ...
                for n_chain in range(0,input.n_chains):
                    ...
                    # Determine bounds to check TC starting values and estimate maximum mass loss确定检查TC起始值和估计最大质量损失的界限
                    ...
                    # Check that tempchange mu and sigma is somewhat within bndhigh and bndlow（检查tempchange mu和sigma是否在bndhigh和bndlow之间）
...
#                        tempchange_sigma = (tempchange_boundhigh - tempchange_boundlow) / 6
                    ...
                    # Select data from model to be stored in netcdf（从模型中选择要存储在netcdf中的数据）
                    ...
                    # set columns for other variables（为其他变量设置列）
        ...
        if input.params2opt.sort() == ['tempbias', 'precfactor'].sort():
            ...
            def objective(modelparameters_subset):
                ...
                mb_dif_mwea
                    Returns the difference in modeled vs observed mass balance [mwea]
                """
                # Use a subset of model parameters to reduce number of constraints required（使用模型参数的子集来减少所需的约束数）
                modelparameters[2] = modelparameters_subset[0]
                modelparameters[3] = modelparameters_subset[1]
                modelparameters[4] = modelparameters_subset[2]
                modelparameters[5] = modelparameters[4] / ddfsnow_iceratio
...
#            observed_massbal_err = glacier_cal_data.loc[cal_idx,'mb_mwe_err'] / (t2 - t1)
            ...
            if icethickness_initial.max() > 0:
                ...
                # Otherwise, run the calibration
                    ...
                    if input.params2opt.sort() == ['tempbias', 'precfactor'].sort():
                        ...
                        #  to ensure this is not the case.（他计算的变量变化量（雅可比）可能太小，这导致#最小化认为它已经达到局部极小值并停止。因此，调整epsilon
                        # #以确保情况并非如此。）
    ...
    #%%
    # Option 1: mimize mass balance difference using multi-step approach to expand solution space（用多步法模拟质量平衡差扩展解空间）
        ...
        def objective(modelparameters_subset):
            ...
            -------
            sum_abs_zscore
                Returns the sum of the absolute z-scores, which represents how well the model matches observations（返回绝对z值的总和，表示模型与观察值的匹配程度）
            """
            # Use a subset of model parameters to reduce number of constraints required（使用模型参数的子集来减少所需的约束数）
            modelparameters[2] = modelparameters_subset[0]
            modelparameters[3] = modelparameters_subset[1]
...
#                             bin_area_subset[:,np.newaxis]).sum() / bin_area_subset.sum())
        ...
        # ==============================================================
        # ===== Individual glacier optimization using objective minimization =====（基于目标极小化的单个冰川优化）
        # Output
        ...
        for n in range(cal_individual_glacno.shape[0]):
            ...
            while continue_loop:
                ...
                # Update model parameters if significantly better（如果明显更好，请更新模型参数）
        ...
        # Export (i) main_glac_rgi w optimized model parameters and glacier-wide climatic mass balance,（主要的优化模式参数和冰川范围的气候质量平衡，）
        #        (ii) comparison of model vs. observations
        # Concatenate main_glac_rgi, optimized model parameters, glacier-wide climatic mass balance（连接主冰川，优化模型参数，冰川气候质量平衡）
...
#        list_packed_vars.append([main_glac_rgi_raw, gcm_name])
    ...
    # If group data is included, then add group dictionary and add group name to main_glac_rgi（如果包含组数据，则添加组字典并将组名称添加到主\u glac\rg）
    ...
    # Air temperature standard deviation [K]（气温标准差）
    ...
    # Pack variables for multiprocessing（打包多处理变量）
    list_packed_vars = []
    n = 0
