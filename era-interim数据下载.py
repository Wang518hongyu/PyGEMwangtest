...
class eraint_variable2download():
    ...
    def __init__(self, vn):
        ...
        date_list = "/".join([d.strftime('%Y%m%d') for d in
                              pd.date_range(start=pygem_prms.eraint_start_date, end=pygem_prms.eraint_end_date,
                                            freq='MS')])
        # Variable name
        self.vn = vn
        # Variable properties
        if self.vn == 'temperature':
            self.properties = {
                ...
                "date": date_list,
                "expver": "1",
                "grid": pygem_prms.grid_res,
                "levtype": "sfc",
                "param": "167.128",
                "area": pygem_prms.bounding_box,
                "stream": "moda",
                "type": "an",
                "format": "netcdf",
                "target": pygem_prms.eraint_fp + pygem_prms.eraint_temp_fn,
            }
            ...
            self.properties = {
                ...
                "type": "fc",
                "format": "netcdf",
                "target": pygem_prms.eraint_fp + pygem_prms.eraint_prec_fn,
            }
            ...
            self.properties = {
                ...
                "type": "an",
                "format": "netcdf",
                "target": pygem_prms.eraint_fp + pygem_prms.eraint_elev_fn,
            }
            ...
            self.properties = {
                ...
                "type": "an",
                "format": "netcdf",
                "target": pygem_prms.eraint_fp + pygem_prms.eraint_pressureleveltemp_fn,
            }
...
if not os.path.isdir(pygem_prms.eraint_fp):
    os.makedirs(pygem_prms.eraint_fp)
# Open server
server = ECMWFDataServer()
# Download data for each variable
for varname in pygem_prms.era_varnames:
...
temp = xr.open_dataset(pygem_prms.eraint_fp + pygem_prms.eraint_temp_fn)
# Netcdf file for lapse rates ('w' will overwrite existing file)
if not os.path.isfile(pygem_prms.eraint_fp + pygem_prms.eraint_lr_fn):
    netcdf_output = nc.Dataset(pygem_prms.eraint_fp + pygem_prms.eraint_lr_fn, 'w', format='NETCDF4')
    # Global attributes
    ...
    startdate = pygem_prms.eraint_start_date[0:4] + '-' + pygem_prms.eraint_start_date[4:6] + '-' + pygem_prms.eraint_start_date[6:]
    enddate = pygem_prms.eraint_end_date[0:4] + '-' + pygem_prms.eraint_end_date[4:6] + '-' + pygem_prms.eraint_end_date[6:]
    ...
    if pygem_prms.option_lr_method == 1:
        # Compute lapse rates from temperature pressure level data
        data = xr.open_dataset(pygem_prms.eraint_fp + pygem_prms.eraint_pressureleveltemp_fn)
        ...
        elev = -pygem_prms.R_gas * pygem_prms.temp_std / (pygem_prms.gravity * pygem_prms.molarmass_air) * np.log(
            levels / pygem_prms.pressure_std)
        # Compute lapse rate
    ...
    elif pygem_prms.option_lr_method == 2:
        # Compute lapse rates from temperature and elevation of surrouding pixels
        # Elevation data
        geopotential = xr.open_dataset(pygem_prms.eraint_fp + pygem_prms.eraint_elev_fn)
