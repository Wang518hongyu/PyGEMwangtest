...
#            self.lr_fp = input.cmip5_fp_lr
    ...
    def importGCMfxnearestneighbor_xarray(self, filename, vn, main_glac_rgi):
        """
        Import time invariant (constant) variables and extract nearest neighbor.（导入时不变（常数）变量并提取最近邻）
        
        Note: cmip5 data used surface height, while ERA-Interim data is geopotential（cmip5数据使用地面高度，而ERA临时数据是位势）
        ...
        # If time dimension included, then set the time index (required for ERA Interim, but not for CMIP5 or COAWST)（如果包含时间维度，则设置时间索引（ERA临时需要，但CMIP5或COAWST不需要））
...
#                    glac_variable[glac] = data[vn][lat_nearidx[glac], lon_nearidx[glac]].values
        # Correct units if necessary (CMIP5 already in m a.s.l., ERA Interim is geopotential [m2 s-2])（如有必要，纠正单位（CMIP5已在m a.s.l.中，ERA临时为位势[m2 s-2]））
        if vn == self.elev_vn:
