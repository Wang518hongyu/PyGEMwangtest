""" List of functions used to set up different aspects of the model 用于设置模型不同方面的函数列表"""
...
def datesmodelrun(startyear=input.ref_startyear, endyear=input.ref_endyear, spinupyears=input.spinupyears,
    ...
    dates_table : pd.DataFrame
        ...
        （表中的每一行是一个时间步，每一列是该时间步的属性（日期、年份、水年等））
    """
    # Include spinup time in start year
    startyear_wspinup = startyear - spinupyears
    # Convert start year into date depending on option_wateryear（根据选项“wateryear”将开始年份转换为日期）
    if option_wateryear == 1:
        ...
        exit()
    # Convert input format into proper datetime format（将输入格式转换为正确的日期时间格式）
    ...
    # Generate dates_table using date_range function使用日期范围函数生成日期表
    if input.timestep == 'monthly':
        # Automatically generate dates from start date to end data using a monthly frequency (MS), which generates
        # monthly data using the 1st of each month 使用每月频率（MS）自动生成从开始日期到结束数据的日期，该频率使用每个月的第一个月生成月度数据
        dates_table = pd.DataFrame({'date' : pd.date_range(startdate, enddate, freq='MS')})
        # Select attributes of DateTimeIndex (dt.year, dt.month, and dt.daysinmonth) 选择DateTimeIndex的属性(日期年份, dt.月，和日期月份)
        ...
        # Remove leap year days if user selected this with option_leapyear  如果用户使用选项“闰年”选择此选项，则删除闰年天数
...
def hypsometrystats(hyps_table, thickness_table):
    """Calculate the volume and mean associated with the hypsometry data.计算与测高数据相关的体积和平均值。
...
def selectcalibrationdata(main_glac_rgi):
    ...
    balances are stored in a csv file.选择模型运行中具有大地质量平衡的所有冰川的大地质量平衡。大地质量余额存储在csv文件中。
    """
    # Import .csv file
...
def selectglaciersrgitable(glac_no=None,
    ...
    glacier inventory. This function returns the rgi table associated with all of these glaciers.根据RGI定义的区域和冰川数量，
    选择模型运行中使用的所有冰川目录。此函数返回与所有这些冰川相关的rgi表。

    glac_no : list of strings（串）
        list of strings of RGI glacier numbers (e.g., ['1.00001', '13.00001'])（RGI冰川编号字符串列表）
    rgi_regionsO1 : list of integers（整数）
        list of integers（整数） of RGI order 1 regions (e.g., [1, 13])
...
#        if len(rgi_glac_number) < 50:
        ...
        for i in os.listdir(rgi_fp):
            if i.startswith(str(region).zfill(2)) and i.endswith('.csv'):
                rgi_fn = i
        try:
            csv_regionO1 = pd.read_csv(rgi_fp + rgi_fn,error_bad_lines=False)
        except:
    ...
    # glacier_table.loc[glacier_table['CenLon'] < 0,'CenLon_360'] = (360 + glacier_table.loc[glacier_table['CenLon'] < 0
    #                                                                ,'CenLON_360'])
    ...
    #   （1）输入具有自定义边界的形状文件，
    #   （2）获取这些边界内冰川的边缘，（3）使用这些可能（或可能不）更能代表区域过程/气候的替代边界进行校准
